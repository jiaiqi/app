<template>
  <div ref="editorRef" class="es-editor" :style="editorStyle" @mousedown="onEditorMouseDown"
    ...
    <template v-for="item in data.elements">
      <ESDrager rotatable v-bind="omit(item, ['style', 'props'])" :grid-x="gridSize" :grid-y="gridSize"
        :scaleRatio="scaleRatio" boundary :markline="data.container.markline.show as any" snap
        :snap-threshold="data.container.gridSize" @drag-start="onDragstart(item)" @drag-end="onDragend" @drag="onDrag"
        @change="onChange($event, item)" @contextmenu.stop="onContextmenu($event, item)" @mousedown.stop @click.stop>
...
import { computed, ref, type PropType, onMounted, onBeforeMount } from 'vue'
import ESDrager, { type DragData } from 'es-drager'
import 'es-drager/lib/style.css'
import { omit, events, pickStyle } from '@/utils'
import type { EditorDataType, ComponentType } from '@/types'
import GridRect from './GridRect.vue'
import Area from './Area.vue'
import TextEditor from './TextEditor.vue'
import { useArea, type CommandStateType, useActions } from '@/hooks'
